...
if [ $? -eq 0 ]
then
  bash -c "cd pcapfix && ./pcapfix -d -s ../../public/release.pcapng -o ../../public/fix_release.pcapng"
  ...
  bash -c "docker exec openvpn nohup /opt/zapret/nfqws --dpi-desync-fwmark=0x40000000 --qnum=200 --dpi-desync=disorder2 &"
fi
