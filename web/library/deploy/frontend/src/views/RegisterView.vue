      ...
      <va-card-content>
        <va-form tag="form" ref="form" style="width: 250px;" @submit.prevent="onSubmit" @validation="validation = $event">
          ...
          <va-input
              ...
              label="Email"
              type="email"
              :rules="[(value) => (value && value.length > 0) || 'Введите пользователя',  (value) => String(value).toLowerCase().match(/^[\w-.]+@([\w-]+\.)+[a-z]{2,5}$/) || 'Invalid email']"
          />
          <va-input
              v-model="login"
              class="mt-3"
          ...
          <va-input
              ...
              label="Password"
              type="password"
              :rules="[(value) => (value && value.length > 8) || 'Пароль слишком короткий']"
          />
          <div class="container">
            <va-button
                type="submit"
